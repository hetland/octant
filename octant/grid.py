...
class CGrid(object):
    ...
    def __init__(self, x, y):
        ...
        assert (np.ndim(x) == 2) and (np.ndim(y) == 2) and \
        ...
        if isinstance(self.x_vert, np.ma.MaskedArray):
            ...
            self.mask_rho = np.asarray(~(~np.bool_(self.mask_rho) | mask),
                                       dtype='d')
    ...
    def _calculate_metrics(self):
        ...
        # domain center
        self.angle[1:-1, 1:-1] = 0.25*(angle_ud[1:-1, 1:] +
                                       angle_ud[1:-1, :-1] +
                                       angle_lr[1:, 1:-1] +
                                       angle_lr[:-1, 1:-1])
        # edges
        self.angle[0, 1:-1] = (1.0/3.0)*(angle_lr[0, 1:-1] +
                                         angle_ud[0, 1:] +
                                         angle_ud[0, :-1])
        self.angle[-1, 1:-1] = (1.0/3.0)*(angle_lr[-1, 1:-1] +
                                          angle_ud[-1, 1:] +
                                          angle_ud[-1, :-1])
        self.angle[1:-1, 0] = (1.0/3.0)*(angle_ud[1:-1, 0] +
                                         angle_lr[1:, 0] +
                                         angle_lr[:-1, 0])
        self.angle[1:-1, -1] = (1.0/3.0)*(angle_ud[1:-1, -1] +
                                          angle_lr[1:, -1] +
                                          angle_lr[:-1, -1])
        #conrers
    ...
    def mask_polygon(self, polyverts, mask_value=0.0):
        ...
        mask = self.mask_rho
        inside = points_inside_poly(np.vstack((self.x_rho.flat,
                                               self.y_rho.flat)).T,
                                    polyverts)
        if np.any(inside):
...
class CGrid_geo(CGrid):
    ...
    lon = property(lambda self: self.lon_vert, None, None,
                   'Shorthand for lon_vert')
    lat = property(lambda self: self.lat_vert, None, None,
...
class Gridgen(CGrid):
    ...
    def generate_grid(self):
        ...
        self._gn = self._libgridgen.gridgen_generategrid2(
            ctypes.c_int(nbry), (ctypes.c_double * nbry)(*self.xbry),
            (ctypes.c_double * nbry)(*self.ybry),
            (ctypes.c_double * nbry)(*self.beta), ctypes.c_int(self.ul_idx),
            ctypes.c_int(self.nx), ctypes.c_int(self.ny), ngrid, xgrid,
            ygrid, ctypes.c_int(self.nnodes), ctypes.c_int(self.newton),
            ctypes.c_double(self.precision),
            ctypes.c_int(self.checksimplepoly), ctypes.c_int(self.thin),
            ctypes.c_int(self.nppe), ctypes.c_int(self.verbose),
            ctypes.byref(nsigmas), ctypes.byref(sigmas), ctypes.byref(nrect),
            ctypes.byref(xrect), ctypes.byref(yrect))
...
class edit_mask_mesh(object):
    ...
    def __init__(self, xv, yv, mask, **kwargs):
        ...
        for dx, dq in zip(xv.shape, mask.shape):
            assert dx == dq+1, '''xv and yv must be cell verticies
        ...
        self._xc = 0.25*(xv[1:, 1:] + xv[1:, :-1] +
                         xv[:-1, 1:] + xv[:-1, :-1])
        self._yc = 0.25*(yv[1:, 1:] + yv[1:, :-1] +
                         yv[:-1, 1:] + yv[:-1, :-1])
